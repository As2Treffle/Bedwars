...
import fr.bedwars.NPC.RightClickNPCEvent;
...
public class PacketReader
{
    ...
    public void readPacket(Player player, Packet<?> packet)
    {
        ...
        if(packet.getClass().getSimpleName().equalsIgnoreCase("PacketPlayInUsentity"))
        {
            ...
            if(getValue(packet, "action").toString().equalsIgnoreCase("INTERACT"))
            {
                for(EntityPlayer npc : NPC.getNPCs())
                {
                    if(npc.getId() == id)
                    {
                        Bukkit.getScheduler().scheduleSyncDelayedTask(BedwarsMain.getPlugin(BedwarsMain.class), new Runnable()
                        {
                            @Override
                            public void run()
                            {
